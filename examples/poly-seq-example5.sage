...
## finite summation formula over the Stirling number triangles 
...
def poly_seq_func(n): 
     ...
     for i in range(0, n + 1):
          poly += (-5 ** i) * S2_func(n+3, i+2) * S1_func(n+3, n+2-i) \
                   * (x ** i)
     ##
     return poly
poly_seq_func_hold = lambda n: poly_seq_func(n)
pseq_data = map(poly_seq_func_hold, range(1, 6))
guess_polynomial_sequence(pseq_data, x, seq_factors = ["S2", "S1"], \
                          index_offset_params = [(0, 1), (0, -1)], 
                          index_offsets = [-1, 0, 1, 2, 3])
